...
def load_highscore():
    ...
    Если файл не существует, создает файл и возвращает начальное значение 0."""
    file_name = "high_score.txt"

    # Проверка наличия файла
    if not os.path.exists(file_name):
        # Если файла нет, создать файл и записать начальное значение 0
        with open(file_name, "w") as file:
            file.write("0")
        return 0

    # Если файл существует, загрузить и вернуть значение рекорда из файла
...
def stone_generator(quantity_of_stones):
    """Генератор падающих камней.

        Args:
            quantity_of_stones: количество камней"""
